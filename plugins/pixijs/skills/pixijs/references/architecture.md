...
## Table of Contents
- [Architecture](#architecture) — Major components, extensions, extension types
- [Environments](#environments) — Browser, Web Workers, custom adapters
- [Garbage Collection](#garbage-collection) — destroy(), texture.unload(), TextureGCSystem
- [Performance Tips](#performance-tips) — General, sprites, graphics, textures optimization
- [Render Groups](#render-groups) — GPU-optimized grouping for static content
- [Render Layers](#render-layers) — Decoupled visual order from logical hierarchy
- [Render Loop](#render-loop) — Frame lifecycle, ticker updates, scene graph rendering
- [Scene Graph](#scene-graph) — Parent-child hierarchy, transforms, culling, render order
