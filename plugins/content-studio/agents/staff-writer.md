...
Read `.claude/content-studio.local.md` for `voice_guide_path`, `output_dir` (default `./drafts`), `default_format` (default `markdown`), and optionally `slug_prefix` and `author`.

Read the voice guide. The high-value sections for drafting:
...
- `mkdir -p` `output_dir` if it doesn't exist.
- File path: `<output_dir>/<slug><ext>`. If the path exists, append `-2`, `-3`, etc. Never overwrite.
- Use `Write`.
