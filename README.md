...
| `content-studio` | `/content-studio:init` | Scaffold a brand voice guide and plugin settings — writes `brand-voice.md` and `.claude/content-studio.local.md`. Plugin also ships a six-agent writer's room (`managing-editor`, `editor-in-chief`, `story-editor`, `staff-writer`, `copy-editor`, `headline-editor`) that wraps the skills with role-specific philosophy |
